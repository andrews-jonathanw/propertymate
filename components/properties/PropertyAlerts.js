    ...
    <div className='absolute top-0 left-0 p-2'>
      {hasUnits ? (
        <div className='flex items-center justify-center'>
          {vacantUnitsCount > 0 ? (
            <>
              ...
              <span className='ml-1 text-sm text-white'>{`${vacantUnitsCount} Vacancies`}</span>
            </>
          ) : (
            <>
              <BsPersonFillCheck className='text-green-600' />
              <span className='ml-1 text-sm text-white'>No Vacancy</span>
            </>
          )}
        </div>
      ) : (
        <div className={`flex items-center justify-center text-${property.occupied ? 'green' : 'yellow'}-600`}>
          ...
          <span className='ml-1 text-sm text-white'>{property.occupied ? 'Occupied' : 'Vacant'}</span>
        </div>
      )}
    </div>
  );
